...
public class CuriosPaperAPIImpl implements CuriosPaperAPI {
  ...
  public boolean isValidAccessory(ItemStack itemStack, String slotType) {
    ...
    String[] types = requiredType.split(",\\s*");
    for (String type : types) {
      if (slotType.equalsIgnoreCase(type.trim())) {
        return true;
      }
    }
  ...
  public ItemStack tagAccessoryItem(ItemStack itemStack, String slotType, boolean addLore) {
    ...
    String[] slots = slotType.toLowerCase().split(",\\s*");
    for (String slot : slots) {
      if (!isValidSlotType(slot.trim())) {
        throw new IllegalArgumentException("Invalid slot type: " + slot.trim());
      }
    }
    ...
    if (meta != null) {
      PersistentDataContainer container = meta.getPersistentDataContainer();
      String normalizedSlotType = String.join(", ", slots);
      container.set(slotTypeKey, PersistentDataType.STRING, normalizedSlotType);

      // Mark each slot as active in the registry so the main GUI shows the button
      for (String s : slots) {
        plugin.getConfigManager().markSlotActive(s.trim());
      }
  ...
  public boolean registerSlot(String slotType, String displayName, org.bukkit.Material icon,
    ...
    boolean added = plugin.getConfigManager().addSlotConfiguration(slotType, config);
    return added;
  }
  ...
  @Override
  public boolean setItemModelConfig(String itemId, boolean modelEnabled, String modelItem,
      Integer customModelData, String itemModel, Float pitchUpLimit, Float pitchDownLimit) {
    org.bg52.curiospaper.manager.ItemDataManager itemDataManager = plugin.getItemDataManager();
    if (itemDataManager == null) {
      plugin.getLogger().warning(
          "ItemDataManager not initialized. Cannot configure 3D model for item: " + itemId);
      return false;
    }

    org.bg52.curiospaper.data.ItemData itemData = itemDataManager.getItemData(itemId);
    if (itemData == null) {
      plugin.getLogger().warning("Item '" + itemId + "' not found. Cannot configure 3D model.");
      return false;
    }

    itemData.setModelEnabled(modelEnabled);
    itemData.setModelItem(modelItem);
    itemData.setModelCustomModelData(customModelData);
    itemData.setModelItemModel(itemModel);
    itemData.setPitchUpLimit(pitchUpLimit);
    itemData.setPitchDownLimit(pitchDownLimit);

    return itemDataManager.saveItemData(itemId);
  }

  @Override
  public boolean setMobDropModelConfig(String itemId, String entityType, boolean modelEnabled, String modelItem,
      Integer customModelData, String itemModel) {
    org.bg52.curiospaper.manager.ItemDataManager itemDataManager = plugin.getItemDataManager();
    if (itemDataManager == null) {
      plugin.getLogger().warning(
          "ItemDataManager not initialized. Cannot configure mob drop 3D model for item: " + itemId);
      return false;
    }

    org.bg52.curiospaper.data.ItemData itemData = itemDataManager.getItemData(itemId);
    if (itemData == null) {
      plugin.getLogger().warning("Item '" + itemId + "' not found. Cannot configure mob drop 3D model.");
      return false;
    }

    boolean found = false;
    for (org.bg52.curiospaper.data.MobDropData drop : itemData.getMobDrops()) {
      if (drop.getEntityType().equalsIgnoreCase(entityType)) {
        drop.setModelEnabled(modelEnabled);
        drop.setModelItem(modelItem);
        drop.setModelCustomModelData(customModelData);
        drop.setModelItemModel(itemModel);
        found = true;
      }
    }
