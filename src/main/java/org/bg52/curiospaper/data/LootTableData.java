...
public class LootTableData {
  ...
  public boolean isValid() {
    ...
    try {
      // Check if it's a valid vanilla loot table (enum-style, legacy)
      LootTables.valueOf(lootTableType.toUpperCase());
      return true;
    } catch (IllegalArgumentException ignored) {
      // Not a legacy enum name, check if it's a valid namespaced key format
      if (lootTableType.contains(":") || lootTableType.contains("/")) {
        // Basic format check: looks like a namespaced key or a path
        return lootTableType.length() < 256; 
      }
    }
