...
/**
 * LootTableFetcher tuned to return ALL available loot tables.
 * Keeps the same robust multi-strategy fetching, but exposes
 * vanilla, datapack, and addon loot tables (chests, mobs, gameplay, etc.).
 */
public final class LootTableFetcher {
  ...
  public static List<NamespacedKey> fetchAllLootTableKeys(JavaPlugin plugin) {
    ...
    try {
      ...
      List<NamespacedKey> baseline = Arrays.stream(LootTables.values())
          ...
          .filter(LootTableFetcher::isValidLootTable)
            ...
            for (Method candidate : zeroMapMethods) {
              try {
                ...
                if (ret instanceof Map) {
                  ...
                  if (added > 0) {
                    plugin.getLogger().info("[LootTableFetcher] Added " + added + " keys via field '"
      ...
      plugin.getLogger().info("[LootTableFetcher] Total loot table keys discovered: " + finalList.size());
      return finalList;
    } catch (Throwable t) {
  ...
  // Add allowed keys from map into discovered set. Returns number added.
  private static int addAllowedKeysFromMap(JavaPlugin plugin, Map<?, ?> map, Set<NamespacedKey> discovered) {
    int added = 0;
